...
import { WorkflowExecutionStatus, WorkflowStatus } from '@/types/worklflow';
...
function LastRunDetail({ workflow }: { workflow: Workflow }) {
  const isDraft = workflow.status === WorkflowStatus.DRAFT;
  if (isDraft) return null;
  const { lasRunAt, lastRunStatus, lastRunId, nextRunAt } = workflow;
  const formattedStartedAt =
    lasRunAt && formatDistanceToNow(lasRunAt, { addSuffix: true });
